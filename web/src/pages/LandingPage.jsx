import { Link } from 'react-router-dom';
import {
  Upload, Play, ShieldCheck, MessageCircleMore, Cpu,
  BrainCircuit, UserCheck, ArrowRight, Shield, Lock, BarChart3
} from 'lucide-react';
import Navbar from '../components/Navbar';

export default function LandingPage() {
  return (
    <div className="page-enter" style={{ minHeight: '100vh', position: 'relative' }}>
      {/* Perspective grid background */}
      <div className="bg-grid" />

      <Navbar />

      {/* ══════ HERO ══════ */}
      <section style={{
        maxWidth: 1280,
        margin: '0 auto',
        padding: '60px 52px 40px',
        display: 'grid',
        gridTemplateColumns: '1.1fr 1fr',
        alignItems: 'center',
        gap: 24,
        minHeight: 'calc(100vh - 240px)',
        position: 'relative',
        zIndex: 5,
      }}>
        {/* ── Left: Text + CTAs ── */}
        <div>
          <div className="tag" style={{ marginBottom: 28 }}>
            <span style={{ fontSize: 16 }}>✦</span> Your Conversations, Your AI
          </div>

          <h1 style={{ marginBottom: 20 }}>
            Meet Your<br />
            <span className="gradient-text">AI Clone</span>
          </h1>

          <p style={{
            fontSize: '1.0625rem', lineHeight: 1.7,
            color: 'var(--color-text-secondary)',
            maxWidth: 460, marginBottom: 40,
          }}>
            Signet creates a personalized AI clone from your
            chat history — built to reflect the way they truly talk.
          </p>

          <div style={{ display: 'flex', gap: 14, marginBottom: 36 }}>
            <Link to="/upload">
              <button className="btn btn-primary" id="hero-upload-btn">
                <Upload size={18} /> Upload Your Chat
              </button>
            </Link>
            <button className="btn btn-ghost" id="hero-demo-btn">
              <Play size={16} fill="#6C5CE7" color="#6C5CE7" /> Watch Demo
            </button>
          </div>

          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
            <ShieldCheck size={22} color="#6C5CE7" style={{ flexShrink: 0, marginTop: 1 }} />
            <p style={{ fontSize: '0.9375rem', lineHeight: 1.6, color: 'var(--color-text-secondary)' }}>
              Your data stays private and secure.<br />
              We never store your conversations.
            </p>
          </div>
        </div>

        {/* ── Right: 3D Orb + Glassmorphism Cards ── */}
        <div style={{
          position: 'relative',
          height: 520,
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          /* Prevent overflow clipping */
          overflow: 'visible',
        }}>
          {/* Dashed connector ring */}
          <div className="connector-ring" />

          {/* Connector dots */}
          <div className="connector-dot" style={{ top: '6%', left: '38%' }} />
          <div className="connector-dot" style={{ top: '22%', right: '8%' }} />
          <div className="connector-dot" style={{ bottom: '32%', left: '12%' }} />
          <div className="connector-dot" style={{ bottom: '10%', right: '32%' }} />

          {/* 3 Concentric Glass Platforms */}
          <div className="orb-platform-1" />
          <div className="orb-platform-2" />
          <div className="orb-platform-3" />

          {/* 3D Orb */}
          <div className="orb">
            <div className="orb-eyes">
              <div className="orb-eye" />
              <div className="orb-eye" />
            </div>
          </div>

          {/* Glassmorphism Feature Cards — positioned WITHIN container */}
          <div className="glass-card" style={{ position: 'absolute', top: '2%', left: '8%' }}>
            <div className="glass-card-icon"><MessageCircleMore size={18} /></div>
            <div>
              <h4>Your Chat</h4>
              <p>Upload your<br />conversations</p>
            </div>
          </div>

          <div className="glass-card" style={{ position: 'absolute', top: '10%', right: '2%' }}>
            <div className="glass-card-icon"><Cpu size={18} /></div>
            <div>
              <h4>AI Processing</h4>
              <p>We analyze tone,<br />pattern & style</p>
            </div>
          </div>

          <div className="glass-card" style={{ position: 'absolute', bottom: '25%', left: '2%' }}>
            <div className="glass-card-icon"><BrainCircuit size={18} /></div>
            <div>
              <h4>Smart Learning</h4>
              <p>Advanced AI creates<br />your unique clone</p>
            </div>
          </div>

          <div className="glass-card" style={{ position: 'absolute', bottom: '5%', right: '2%' }}>
            <div className="glass-card-icon"><UserCheck size={18} /></div>
            <div>
              <h4>Your AI Clone</h4>
              <p>Talk like them.<br />Respond like them.</p>
            </div>
          </div>
        </div>
      </section>

      {/* ══════ BOTTOM FEATURE BADGES ══════ */}
      <section id="features" style={{
        maxWidth: 1280,
        margin: '0 auto',
        padding: '0 52px 60px',
        position: 'relative',
        zIndex: 5,
      }}>
        <div className="badge-section">
          <div className="glass-badge">
            <div className="glass-badge-icon"><Shield size={22} /></div>
            <div>
              <h4>100% Private</h4>
              <p>Your chats stay on<br />your device.</p>
            </div>
          </div>

          <div className="glass-badge">
            <div className="glass-badge-icon"><UserCheck size={22} /></div>
            <div>
              <h4>AI Clone</h4>
              <p>AI that talks like<br />your person.</p>
            </div>
          </div>

          <div className="glass-badge">
            <div className="glass-badge-icon"><BarChart3 size={22} /></div>
            <div>
              <h4>Smart Analysis</h4>
              <p>Deep insights from<br />your conversations.</p>
            </div>
          </div>

          <div className="glass-badge">
            <div className="glass-badge-icon"><Lock size={22} /></div>
            <div>
              <h4>Secure & Safe</h4>
              <p>We never store or share<br />your data.</p>
            </div>
          </div>
        </div>
      </section>

      {/* Responsive overrides */}
      <style>{`
        @media (max-width: 1024px) {
          section:first-of-type {
            grid-template-columns: 1fr !important;
            text-align: center;
            padding: 32px 24px !important;
          }
          section:first-of-type > div:first-child {
            display: flex;
            flex-direction: column;
            align-items: center;
          }
          section:first-of-type > div:last-child {
            height: 400px !important;
          }
        }
        @media (max-width: 640px) {
          .glass-card { display: none !important; }
          .connector-ring, .connector-dot { display: none !important; }
        }
      `}</style>
    </div>
  );
}
